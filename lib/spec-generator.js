...
/**
 * Generate project specifications from Claude Code logs using OpenAI-compatible API
 */
export class SpecGenerator {
  /**
   * @param {Object} options - Configuration options
   * @param {string} options.apiKey - API key (defaults to OPENAI_API_KEY env var)
   * @param {string} options.baseURL - API base URL (defaults to OPENAI_BASE_URL env var)
   */
  constructor(options = {}) {
  ...
  async processChunk(chunk, previousContext, model, isFirst) {
    ...
    const systemPrompt = `You are analyzing user requirements and requests from a Claude Code session to generate a comprehensive project specification for business analysts.
...
`;
    ...
    // Combine context and user content
...
`;
    ...
    // Make API call using the chat completions API (OpenAI-compatible)
